...
function filterChartPeserta(){
    ...
    const checked = [...document.querySelectorAll('#checkboxContainerPeserta input:checked')]
        .map(cb => cb.value);
...
reader.onload = function(evt){
    ...
    console.log(rawData);   // ⬅ TAMBAHKAN DI SINI
    renderChart(rawData);
};

let pesertaChart;

document.getElementById('uploadFilePeserta')
...
reader.onload = function(evt){
    ...
    const workbook = XLSX.read(data, {type:'array'});
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    pesertaData = XLSX.utils.sheet_to_json(sheet);  // ⬅ jangan pakai const
...
function filterChartPeserta(){
  ...
  function renderChart(data){
    ...
    const program = data.map(d =>
        Number((d["PROGRAM"] || d["Program"] || 0)
        .toString().replace(/[.,]/g,'')));

    const realisasi = data.map(d =>
        Number((d["REALISASI"] || d["Realisasi"] || 0)
        .toString().replace(/[.,]/g,'')));

   const chartId = document.body.classList.contains('page-safety')
    ? 'safetyChart'
    : 'operasiChart';
...
series: [
    {
        name: 'Program',
        data: program,
        color: isSafety ? '#1e3a8a' : '#197d8a'
    },
    {
        name: 'Realisasi',
        data: realisasi,
        color: isSafety ? '#facc15' : '#cfd3bb'
    }
]
